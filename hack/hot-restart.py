#!/usr/bin/env python
#
# Copyright 2019 Envoy Authors, Samaritan Authors
#
...
PARENT_PID_ENV = "__Samaritan_Parent__"
PARENT_TERMINATE_TIME_ENV = "__Samaritan_Parent_Terminate_Time__"
...
def term_all_children():
    ...
    global pid_list
    for pid in pid_list:
        _print("sending TERM to PID={}".format(pid))
        try:
            os.kill(pid, signal.SIGTERM)
        except OSError:
            _print("error sending TERM to PID={} continuing".format(pid))
    ...
    if all_exited:
        _print("all children exited cleanly")
    else:
        for pid in pid_list:
            _print("child PID={} did not exit cleanly, killing".format(pid))
...
def force_kill_all_children():
    ...
    global pid_list
    for pid in pid_list:
        _print("force killing PID={}".format(pid))
        try:
            os.kill(pid, signal.SIGKILL)
        except OSError:
            _print("error force killing PID={} continuing".format(pid))
...
def sigusr1_handler(signum, frame):
    ...
    global pid_list
    for pid in pid_list:
        _print("sending SIGUSR1 to PID={}".format(pid))
        try:
            os.kill(pid, signal.SIGUSR1)
        except OSError:
            _print("error in SIGUSR1 to PID={} continuing".format(pid))
...
def sigchld_handler(signum, frame):
    ...
    for pid in pid_list_copy:
        ...
        if os.WIFEXITED(exit_status):
            ...
            _print("PID={} exited with code={}".format(ret_pid, exit_code))
            if exit_code == 0:
            ...
            _print("PID={} was killed with signal={}".format(
    ...
    if kill_all_and_exit:
        _print("Due to abnormal exit, force killing all child processes and exiting")
    ...
    if not pid_list:
        _print("exiting due to lack of child processes")
...
def update_env(fn):
    def w(*args, **kwargs):
        if pid_list:
            os.environ[PARENT_PID_ENV] = str(pid_list[-1])
        fn(*args, **kwargs)
...
def fork_and_exec():
    ...
    _print("forking and execing new child process at epoch {}".format(restart_epoch))
    restart_epoch += 1
    ...
    if child_pid == 0:
        ...
        # Parent process
        _print("forked new child process with PID={}".format(child_pid))
...
def main():
    ...
    _print("starting hot-restarter with target: [{}]".format(" ".join(sys.argv[1:])))
