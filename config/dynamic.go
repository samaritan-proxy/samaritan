...
//go:generate mockgen -package $GOPACKAGE --destination ./mock_discovery_test.go $REPO_URI/pb/api DiscoveryServiceClient,DiscoveryService_StreamDependencyClient,DiscoveryService_StreamSvcConfigsClient,DiscoveryService_StreamSvcEndpointsClient
...
func (d *dynamicSource) streamSvcs(ctx context.Context) {
	req := &api.DependencyRequest{
		Instance: d.b.Instance,
	}
	stream, err := d.c.StreamDependency(ctx, req)
	if err != nil {
